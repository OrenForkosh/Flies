...
classdef DimReduction
    ...
    methods (Static = true)
        ...
        function ndata = Normalize(data, opt)
            arguments
                data
                opt.epochs = [];
                opt.method = 'warp';
            end
            error("has a bug - doesn't remove nan values and use them while normalizing");
            if isempty(opt.epochs)
                opt.epochs = ones(size(data, 1), 1);
            end
            switch opt.method
                case 'warp'
                    normfunc = @(x) Q.nwarp(x);
                otherwise
                    error("can't set '%s' as a normalization method; valid options are: 'warp'", opt.method);
            end
            ndata = data;
            [~, ~, indices] = unique(opt.epochs);
            for i = 1:max(indices)
                map = indices == i;
                ndata(map, :) = normfunc(data(map, :));
            end
        end
        ...
        function [d, D] = FisherRaoCriterion(x, labels)
            ...
            warning('code changed due to weird bug');
            D = DimReduction.ScatterWithin(x, labels)/DimReduction.ScatterBetween(x, labels);
            d = trace(D);
        end
        ...
        function [W, e, s] = LDAComplex(x, minWithin, minBetween, n, varargin)
%                     function [Sw, Sb] = ScatterComplex(x, minWithin, minBetween, groups, alpha)
            p = inputParser;
            p.addOptional('Groups', []);  
            ...
            [Sw, Sb] = DimReduction.ScatterComplex(x, minWithin, minBetween, opt.Groups, opt.Alpha);
        ...
        function [W, e, s] = invLDA(varargin)
            [W, e, s] = DimReduction.LDA(varargin{:}, 'Inv', true);
        end
        ...
        function Sw = ScatterWithin(x, labels, fix, fast)            
            ...
            if fast
                mu = Q.accumrows(labels, x, @nanmean);
                dx = x - mu(labels, :);
                Sw = dx' * dx;
                return;
            end
        ...
        function Sb = ScatterBetweenExplicit(x, labels)
            %%
            meanAll = mean(x);
            meanClust = Q.accumrows(labels, x, @mean, nan);
            clustSize = Q.accumrows(labels, x, @length, nan);
            valid = ~isnan(meanClust);
            d = (meanClust(valid, :) - meanAll) .* sqrt(clustSize(valid));
            Sb = d' * d;
        end
        ...
        function [Sw, Sb] = ScatterComplex(x, minWithin, minBetween, groups, alpha)
            ...
            for g=1:ng
                map = groups == g;
                uMW = unique(minWithin(map));
                uMB = unique(minBetween(map));
                totalMW = totalMW + length(uMW);
                totalMB = totalMB + length(uMB);
            end

            for g=1:ng
                map = groups == g;
                [~, ~, minWithinIdx] = unique(minWithin(map));
                [~, ~, minBetweenIdx] = unique(minBetween(map));

                swMW = DimReduction.ScatterWithin(x(map, :), minWithinIdx);
                swMB = DimReduction.ScatterWithin(x(map, :), minBetweenIdx);
                
                sbMW = DimReduction.ScatterBetween(x(map, :), swMW);
                sbMB = DimReduction.ScatterBetween(x(map, :), swMB);
                
                nidxMW = max(minWithinIdx);
                nidxMB = max(minBetweenIdx);

                Sw = Sw + swMW * nidxMW / totalMW + alpha * sbMB * nidxMB / totalMB;
                Sb = Sb + sbMW * nidxMW / totalMW + alpha * swMB * nidxMB / totalMB;
            end
