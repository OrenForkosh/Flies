classdef Q
    ...
    methods (Static = true)
        ...
        function system(cmd)
            % it's stupid, but in some cases Matlab need it because this
            % software is broken
            system(cmd);
        end
...
%         function res = accumrowsF(subs, val, fun, fillval)
%             arguments
%                 subs
%                 val
%                 fun = {};
...
%             newsubs = repmat(subs(:), 1, ncols) + (0:ncols - 1) * maxsubs;
%             res = accumarray(newsubs(:), val(:), [], fun{:}, fillval);
%             res = reshape(res, maxsubs, ncols);
%         end
        ...
        function [order, clusters, z] = hiersort(D, opt)
            arguments
                D
                %%
                % Distance metric to use on D. If not specified, assumes D 
                % is the distance matrix. Possible values are like in
                % 'pdist', for example: 'correlation', 'euclidean', 'mahalanobis'	
                opt.metric = []; 
                opt.cutoff = [];
                opt.output = false;
                opt.k = []; % number of clusters
            end
            % runs hierarchical sorting
            if ~isempty(opt.metric)
                D = pdist(D, opt.metric);
            elseif ~isvector(D)
            ...
            if ~isempty(opt.k)
                clusters = cluster(z,'MaxClust', opt.k);
                opt.cutoff = z(end-opt.k, 3);
            elseif isempty(opt.cutoff)
                opt.cutoff = median([z(end-2,3) z(end-1,3)]);
                clusters = cluster(z,'cutoff',opt.cutoff, 'criterion','distance');
            else
                clusters = cluster(z,'cutoff',opt.cutoff, 'criterion','distance');
            end
%             clusters = clusters(order);
            if nargout == 0 || opt.output
                % subplot(3,1, 1);
                % if ~isempty(opt.cutoff)
                %     dendrogram(z,0,'reorder', order, 'ColorThreshold', opt.cutoff);
                % else
                %     dendrogram(z,0,'reorder', order);
                % end
                % subplot(3,1,2:3);
                d = squareform(D);
                d = 1 - d(order, order);
                d(eye(length(d)) == 1) = nan;
                Plot.Hinton(d, range=[0 max(d(:))], colormap=Colormaps.FromTo(Colors.PrettyRed, Colors.PrettyRed));
                set(gca, 'XTick', 1:length(d), 'XTickLabel', order)
                set(gca, 'YTick', 1:length(d), 'YTickLabel', order)
            end
        end
        ...
        function to = cpfield(from, to, names, ignore, default)
...
%                         to.(names) = default;
                ...
                if nargin < 5                    
                    ...
                    for i=1:length(names)
                        to = Q.cpfield(from, to, names{i}, ignore, default{i});
                    end
                end
            end
        end
...
%             X = im2col(Q.torow(x), [1, n+1], 'sliding');
        ...
        function [c, p] = pearsonMetric(varargin)
            % https://arxiv.org/abs/1908.06029
%             c = sqrt(1 - corr(x, y));
%             return
            [c, p] = Q.nancorr(varargin{:});
            c = sqrt(1 - c);
        end
        ...
        function [c, p] = nancorr(x, y, varargin)
            ...
            for i=1:size(x, 2)
                ...
                for j=1:size(y, 2)
                    ...
                    [c(i,j) p(i,j)] = corr(X(valid), Y(valid), varargin{:});
                end
            end
        end
        ...
        function [r, N, o] = rank(x, dim, nranks)
        ...
        function y = normmax(n)
            %% computes the expected maximal value from a normal distribution with n samples (see https://max.pm/posts/max_dist/)
            f = @(y, n) n * y .* normpdf(y) .* normcdf(y).^(n-1);
            y = integral(@(y) f(y, n), -inf, inf);
        end

        function [r, N, n] = rank_v2(x)
            %% similar to rank but handles similar values by mapping them to
            % the smallest number. For example:
            %   Q.rank([1 2 3 3 6]) = [1 2 4 4 5];
            % and 
            %   Q.rank_v2([1 2 3 3 6]) = [1 2 3 3 4];
            [u, ~, r] = unique(x);
            r = reshape(r, size(x));
%             N = length(u);
            N = size(x, 1);
...
%             r = idx;
        ...
        function y = nwarp_v3(s, varargin)
            x = s;
            [~, o] = sort(x);
            [u, ~, ui] = unique(x);
            N = length(x);
            ...
            vals = norminv(linspace(1-inmax, inmax, N));
            p = normpdf(vals);
            y = vals(o);
            for i = 1:length(u)
                idx = ui == i;
                y(idx) = mean(y(idx) .* p(idx));
            end
        ...
        function y = nwarp_v2(x, varargin)
            %%
            [rank, N, n] = Q.rank_v2(x, varargin{:});
            nmax = Q.normmax(N);
            inmax = normcdf(nmax);
            if n == 1
                l = 0;
            else
                l = norminv(linspace(1-inmax, inmax, n));
            end
            y = l(rank);
            y = reshape(y, size(x));
        end
        ...
        function y = nwarpold(x, varargin)
            ...
            y = reshape(y, size(x));
                ...
                function y = nwarpRfixed(x, dim, n)
            if nargin < 2
                dim = 1;
                if isvector(x)
                    x = x(:);
            ...
            rank = Q.rank(x, dim, n);
            mid = sum(~isnan(rank), dim) / 2;
            rank(isnan(x)) = nan;
            nrank = .5 * (rank  ./ mid);
            y = norminv(0.025 + nrank * 0.95);
        end
        ...
        function y = nanznorm(x)
            cx =  bsxfun(@minus, x, nanmean(x));
            stdx = nanstd(x, 0);
            y = bsxfun(@rdivide, cx, stdx);
            y(:, stdx == 0) = 0;
        end        
