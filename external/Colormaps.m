classdef Colormaps
    ...
    methods (Static)
        ...
        function cmap = Gradient(n)
            arguments
                n = 256;
            end
            
            cmap = Colormaps.FromTo(MyColorTools.fromHex('1f3360'), MyColorTools.fromHex('3f63a4'), MyColorTools.fromHex('6190bf'), MyColorTools.fromHex('9fc2db'), MyColorTools.fromHex('9fc2db'), MyColorTools.fromHex('fdfefd'), MyColorTools.fromHex('f3d9c5'), MyColorTools.fromHex('dfa283'), MyColorTools.fromHex('ba5f4e'), MyColorTools.fromHex('961d2e'), MyColorTools.fromHex('5d1329'), n);
        end
        

        function c = Pantone2022
            c = Colors.FromHex('695650', 'C5AECF', 'B9BD2A', 'AC6C29', 'E9728D', 'E7C9C9', 'F8DE8C', '2F74B7', 'F3774C', '76C1B2', 'CA3422', '853F85', '856A57', '01656E', '81A0B9', 'FEC04E', '01589B', 'C43262', 'FAC8C2', 'B4DCEA');
        end
        ...
        %% Basic colormaps %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function cmap = CategoricalSpectrum(n)
            arguments
                n = 15;
            end
            cmap = Colors.FromHex('#4F8DF9', '#53BCFF', '#5FD5D9', '#26B3A6', '#1EC794', '#8ACD77', '#FFD03E', '#FF8100', '#F26F6F', '#E6648F', '#B74DC9', '#625ED9', '#C38A7F', '#556575', '#4D587B', '#AEC7E0');
            % cmap = Colormaps.ExpandToNLevels(cmap, n);
        end

        
        function cmap = CategoricalNeue(n)
            arguments
                n = 15;
            end
            cmap = Colors.FromHex('4F8DF9', '5FD5D9', 'F26F6F', 'FFD03E', '524FA0', 'A6A2FF', '009688', '8ACD77', 'B74DC9', 'F48FB1', 'AD695C', 'AF7A6D', '333232',  'FF8100', 'FFB366');
            cmap = Colormaps.ExpandToNLevels(cmap, n);
        end

        function cmap = CategoricalRainbox()
            cmap = Colors.FromHex('0077c8', '1ca3ec', '00a651', 'fedd00', 'f7941d', 'f26522', 'ee3e34', 'a349a4', '6d6e71', 'f2f2f2');
        end

        function cmap = CategoricalRainbox2()
            cmap = Colors.FromHex('0077c8', '1ca3ec', '00a651', 'fedd00', 'f7941d', 'f26522', 'ee3e34', 'a349a4', 'ff69b4', '8b008b');
        end

        function cmap = CategoricalRainboxRetro()
            cmap = Colors.FromHex('3856b4', '76a6d1', '50c878', 'fedd5b', 'f68d2e', 'd75c37', 'e43b44', '8b5580', 'e98fc9', '6b3fa0')
        end
        ...
        function cmap = Retro(ncolors) % Discrete
            ...
            cmap(7, :) = Colors.ParseHex('2596BE');
            cmap(8, :) = Colors.ParseHex('9925BE');
            cmap(9, :) = Colors.ParseHex('BE4D25');
            cmap(10, :) = Colors.ParseHex('49BE25');
            cmap(11, :) = Colors.ParseHex('145369');
        ...
        function cmap = Random(nlevels)
            ...
            r(:, 1, 2) = .5 + .5 * r(:, 1, 2);
            r(:, 1, 3) = .5 + .5 * r(:, 1, 3);
            cmap = reshape(hsv2rgb(r), [nlevels, 3]);
            end
        end
    ...
    methods (Static)
        ...
        function cmap = ExpandToNLevels(cmap, n)
            arguments
                cmap
                n = 256;
            end
        ...
        function cmap = FromTo(varargin)
            ...
            for i=2:length(varargin)
                src = varargin{i-1};
                dst = varargin{i};
                c = Colormaps.makeColorMap(src,dst,round(pos*(i-1) - size(cmap, 1)));
